...
static Errors_of_ASM constructor(struct Special_elements_for_processing *elements)
{
    ...
    elements->is_current_function_has_caller = false;
...
static void process_operator_while(struct Node *root, FILE *file_pointer, struct Special_elements_for_processing *elements)
{
    ...
    while ((elements->stack_while).size != 0)
    {
        ...
        printf("after stack_pop in process_operator_while\n");
        if (error != NO_ERRORS)
        {
            fprintf(stderr, "error of stack = %d\n", error);
            abort();
...
static void process_operator_if(struct Node *root, FILE *file_pointer, struct Special_elements_for_processing *elements)
{
    ...
    if ((elements->stack_if).size != 0)
    {
        ...
        printf("after stack_pop in process_operator_if\n");
        if (error != NO_ERRORS)
        {
            fprintf(stderr, "error of stack = %d\n", error);
            abort();
...
static void process_built_in_function(struct Node *root, FILE *file_pointer, struct Special_elements_for_processing *elements)
{
    ...
    //printf("element in process_built_in_function = %lf\n", element);
    if (error != NO_ERRORS)
    {
        fprintf(stderr, "error of stack = %d\n", error);
        abort();
    ...
    error = stack_pop(&(elements->stack_for_last_commands), &element);
    if (error != NO_ERRORS)
    {
        fprintf(stderr, "error of stack = %d\n", error);
        abort();
...
static void process_definition_of_function(struct Node *root, FILE *file_pointer, struct Special_elements_for_processing *elements)
{
    ...
    // if (((elements->all_functions)[id]).has_caller)
    // {
    ...
    if (elements->is_current_function_has_caller)
    {
        elements->is_current_function_has_caller = false;
        ...
        //printf("after stack_pop in process_definition_of_function\n");
        if (error != NO_ERRORS)
        {
            fprintf(stderr, "error of stack = %d\n", error);
            abort();
...
static void find_caller_of_function(struct Node *root, char *function_name, struct Special_elements_for_processing *elements, struct Variable *function_parametres)
{
    ...
    if ((root->value).type == OPERATOR && (root->value).operator_ == OPERATOR_ASSIGNMENT)
    {
        ...
        //printf("(elements->processed_variables)[%lu].variable_name = %s\n", index, (elements->processed_variables)[index].variable_name);
        Type_of_number type = INT;
        get_result_type(root->right, &type);
        //printf("(elements->processed_variables)[%lu].type_of_variable = %d\n", index, type);
    ...
    {
        elements->is_current_function_has_caller = true;
        Stack_Elem_t element = 0;
        Errors error = stack_element(&(elements->current_function), &element);
        size_t id = (size_t)element;
        if (error != NO_ERRORS)
...
static void save_global_variable(struct Node *root, FILE *file_pointer, struct Special_elements_for_processing *elements)
{
    ...
    // printf("((root->left)->value).variable.variable_name = %s\n", ((root->left)->value).variable.variable_name);
    // printf("element = %lf\n", element);
    int ram_address = -1;
    bool is_exists = false;
    int empty_index = 0;
    ...
    if (!is_exists)
    {
        if (elements->is_any_double_number && element == DOUBLE)
        {
            last_address -= 4;
...
static void process_expression_after_assignment(struct Node *root, FILE *file_pointer, struct Special_elements_for_processing *elements, size_t *counter_of_parametres, Value_type type)
{
    ...
    switch ((root->value).type)
    {
        ...
        case OPERATION:
        {
            ...
            // printf("after stack_pop in case operation for element1 in process_expression_after_assignment\n");
            // printf("element1 = %lf\n", element1);
            if (error != NO_ERRORS)
            {
                fprintf(stderr, "error of stack = %d\n", error);
            ...
            // printf("after stack_pop in case operation for element2 in process_expression_after_assignment\n");
            if (error != NO_ERRORS)
            {
                fprintf(stderr, "error of stack = %d\n", error);
            }
...
static void process_global_variable(struct Value *value, FILE *file_pointer, struct Special_elements_for_processing *elements)
{
    ...
    //printf("((elements->all_variables)[index]).type_of_variable = %d\n", ((elements->all_variables)[index]).type_of_variable);
...
static void process_operation_for_double(struct Value *value, FILE *file_pointer, struct Special_elements_for_processing *elements)
{
    ...
    //printf("after stack_pop in process_operation_for_double for the first_elem\n");
    if (error != NO_ERRORS)
    {
        fprintf(stderr, "error of stack = %d\n", error);
        abort();
    ...
    //printf("after stack_pop in process_operation_for_double for the second_elem\n");
    if (error != NO_ERRORS)
    {
        fprintf(stderr, "error of stack = %d\n", error);
        abort();
...
static void process_operation_for_int(struct Value *value, FILE *file_pointer, struct Special_elements_for_processing *elements)
{
    ...
    Stack_Elem_t second_elem = 0;
    Errors error = stack_pop(&(elements->stack_for_last_commands), &first_elem);
    //printf("after stack_pop in process_operation_for_int for the first_elem\n");
    if (error != NO_ERRORS)
    {
        fprintf(stderr, "error of stack = %d\n", error);
        abort();
    ...
    error = stack_pop(&(elements->stack_for_last_commands), &second_elem);
    //printf("after stack_pop in process_operation_for_int for the second_elem\n");
    if (error != NO_ERRORS)
    {
        fprintf(stderr, "error of stack = %d\n", error);
        abort();
